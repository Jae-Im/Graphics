...
{ 
    ...
    { 
        ...
        this.materials =
        {
            ...
            stand: context.get_instance(Phong_Shader).material(Color.of(0, 0, 0, 1), {ambient: 1 , texture: context.get_instance("assets/wood.jpg", true)}),
            bump_material: context.get_instance(Bump_Shader)
                  .material(Color.of( .35,.35,.35,1 ), 
                  {ambient:1, specularity:0,
                  texture:
                    { diffuse:context.get_instance("assets/sandclean.png", true), 
                      normal:context.get_instance("assets/sand_normal.png", true)}})
        }
        ...
        this.totalmon = (testingMode) ? 10 : 5;
        this.foodamt = (testingMode) ? 1 : 0;
    ...
    { 
        ...
        this.key_triggered_button( "Right", ["l"], () => {
           ...
           if( this.rotatecount % 4 == 0 && this.feed_area_x + 0.5 < 14){
               this.feed_area_x += 0.5;
           }

           if( this.rotatecount % 4 == 1 && this.feed_area_z - 0.5 > -14){
               this.feed_area_z -= 0.5;
           }

           if( this.rotatecount % 4 == 2 && this.feed_area_x - 0.5 > -14){
               this.feed_area_x -= 0.5;
           }

           if( this.rotatecount % 4 == 3 && this.feed_area_z + 0.5 < 14){
               this.feed_area_z += 0.5;
           }
        });
        
        this.key_triggered_button( "Down", ["k"], () => {
            if( this.rotatecount % 4 == 0 && this.feed_area_z + 0.5 < 14){
               this.feed_area_z += 0.5;
           }

           if( this.rotatecount % 4 == 1 && this.feed_area_x + 0.5 < 14){
               this.feed_area_x += 0.5;
           }

           if( this.rotatecount % 4 == 2 && this.feed_area_z - 0.5 > -14){
               this.feed_area_z -= 0.5;
           }

           if( this.rotatecount % 4 == 3 && this.feed_area_x - 0.5 > -14){
               this.feed_area_x -= 0.5;
           }
        });
    ...
    {   
        ...
        //Update time left
        if (Math.floor(t) <= gameTime && !testingMode)
            this.timerem = gameTime - Math.floor(t);
        ...
        for (let i = 0; i < this.fish.length; i++)
        {
            ...
            if(this.fish[i].health <= 0 && !testingMode)
            {
    ...
    draw_tank(graphics_state, obtain_shadow_map)
    {
		...
		//Draw ground
		this.shapes.wall.draw(graphics_state, model_transform.times(Mat4.translation([0, -10, 0])).times(Mat4.scale([15, 0.05, 15])), groundMaterial);
